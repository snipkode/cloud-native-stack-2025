...
import { PricingCalculator, PricingDetails } from './pricing';
...
export class BillingManager {
  ...
  async updatePlan(planId: string, updateData: Partial<Plan>): Promise<Plan | null> {
  ...
  async createSubscription(request: CreateSubscriptionRequest): Promise<Subscription> {
    ...
    const existingPlanSubscription = existingSubscriptions.find(
      sub => sub.planId === request.planId && sub.status === SubscriptionStatus.ACTIVE
    );
    ...
    const subscription: Subscription = {
      ...
      status: request.trialEnd ? SubscriptionStatus.TRIALING : SubscriptionStatus.ACTIVE,
      startDate: startDate,
      currentPeriodStart: startDate,
      currentPeriodEnd: currentPeriodEnd,
      trialEnd: request.trialEnd || (plan.trialPeriodDays > 0 ? moment(now).add(plan.trialPeriodDays, 'days').toDate() : undefined),
    ...
    if (coupon) {
      ...
      if (coupon.duration === 'repeating' && coupon.durationInMonths) {
        discount.end = moment(now).add(coupon.durationInMonths, 'months').toDate();
  ...
  async updateSubscription(subscriptionId: string, request: UpdateSubscriptionRequest): Promise<Subscription | null> {
  ...
  async cancelSubscription(subscriptionId: string, reason?: string): Promise<Subscription | null> {
    const subscription = await this.storage.getSubscription(subscriptionId);
    if (!subscription) return null;
  ...
  async createInvoice(request: CreateInvoiceRequest): Promise<Invoice> {
    ...
    const pricingDetails = PricingCalculator.calculateInvoicePrice(
      request.lineItems.map(item => ({...item})), // Copy the line items
      customer,
      this.config.taxRate || 0  // Pass the default tax rate
    );
  ...
  async createInvoiceForSubscription(subscriptionId: string): Promise<Invoice | null> {
    const subscription = await this.storage.getSubscription(subscriptionId);
    if (!subscription) return null;
    ...
    // Calculate pricing using the pricing calculator
    const pricingDetails = PricingCalculator.calculateSubscriptionPrice(
      plan,
      customer,
      this.config.taxRate || 0,  // Pass the default tax rate
      coupon,
    ...
    const invoice: Invoice = {
      ...
      amountPaid: 0,
      currency: plan.currency,
      dueDate: moment(now).add(30, 'days').toDate(), // Default 30 days due
      issueDate: now,
      lineItems,
      tax: pricingDetails.taxAmount,
      subtotal: pricingDetails.subtotal,
  ...
  // Payment processing
  async processPayment(request: ProcessPaymentRequest): Promise<Payment | null> {
  ...
  async createPaymentIntent(request: PaymentIntentRequest): Promise<string | null> {
    if (!this.stripe) {
  ...
  // Usage-based billing
  async recordUsage(subscriptionId: string, metricId: string, quantity: number): Promise<UsageRecord> {
    const subscription = await this.storage.getSubscription(subscriptionId);
    if (!subscription) {
  ...
  async getUsageForSubscription(subscriptionId: string, metricId: string, periodStart: Date, periodEnd: Date): Promise<number> {
  ...
  // Coupon management
  async createCoupon(couponData: Omit<Coupon, 'id' | 'timesRedeemed' | 'valid' | 'createdAt' | 'updatedAt'>): Promise<Coupon> {
    const coupon: Coupon = {
      ...couponData,
      id: generateId(),
      timesRedeemed: 0,
  ...
  // Helper methods
  private calculateNextBillingDate(startDate: Date, plan: Plan): Date {
  ...
  private async handlePlanChange(oldSubscription: Subscription, newPlanId: string): Promise<void> {
    ...
    const prorationResult = PricingCalculator.calculateProration(
      oldPlan,
      newPlan,
      oldSubscription.currentPeriodStart,
      oldSubscription.currentPeriodEnd,
      new Date() // prorated as of today
    );
  ...
  private async calculateUsageCharges(subscriptionId: string): Promise<Array<{description: string; amount: number; quantity: number}>> {
  ...
  async getBillingDashboard(): Promise<{
    ...
    let allActiveSubscriptions: Subscription[] = [];
    for (const plan of plans) {
      const subs = await this.storage.listSubscriptionsByPlan(plan.id);
      allActiveSubscriptions = allActiveSubscriptions.concat(subs.filter(sub => sub.status === SubscriptionStatus.ACTIVE));
    }
