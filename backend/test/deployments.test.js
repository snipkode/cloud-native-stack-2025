...
const API_PREFIX = '/api/deployments';
...
describe('Deployments API Tests', () => {
  ...
  test('should create a new deployment', async () => {
    try {
      ...
      console.log(`Created deployment with ID: ${testDeploymentId}`);
    } catch (error) {
      console.error('Error creating deployment:', error.response?.data || error.message);
      throw error;
    }
  });

  // Test getting all deployments
  test('should get all deployments for the user', async () => {
    try {
  ...
  test('should get a specific deployment by ID', async () => {
    if (!testDeploymentId) {
      throw new Error('No deployment ID available for test');
    }
    ...
    try {
      ...
      console.error('Error getting specific deployment:', error.response?.data || error.message);
      throw error;
    }
  });

  // Test restarting deployment
  test('should restart a deployment', async () => {
    if (!testDeploymentId) {
      throw new Error('No deployment ID available for test');
    }
    ...
    try {
      ...
      console.error('Error restarting deployment:', error.response?.data || error.message);
      throw error;
    }
  });

  // Test stopping deployment
  test('should stop a deployment', async () => {
    if (!testDeploymentId) {
      throw new Error('No deployment ID available for test');
    }
    ...
    try {
      ...
      console.error('Error stopping deployment:', error.response?.data || error.message);
      throw error;
    }
  });
  ...
  test('should get deployment logs', async () => {
    ...
    try {
      ...
      console.error('Error getting deployment logs:', error.response?.data || error.message);
      throw error;
    }
  });

  // Test deleting deployment
  test('should delete a deployment', async () => {
    if (!testDeploymentId) {
      throw new Error('No deployment ID available for test');
    }
    ...
    try {
      ...
      console.error('Error deleting deployment:', error.response?.data || error.message);
      throw error;
    }
  });

  // Verify deletion
  test('should return 404 when accessing deleted deployment', async () => {
    if (!testDeploymentId) {
      throw new Error('No deployment ID available for test');
    }
...
describe('Deployments API Error Handling', () => {
  test('should return 400 for invalid input when creating deployment', async () => {
    try {
      await api.post('/', {
